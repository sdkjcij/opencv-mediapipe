        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if finger_points:
                    ...
                    if hand_judge % 2 == 0:
                        hand_left_right = "right"
                    else:
                        hand_left_right = "left"
                    left_right = hand_left_right
                    text = hand_pos(finger_angle)  # 取得手勢所回傳的內容
                    if left_right == "left":
                        cv2.putText(img, text, (30, 120), fontFace, 5, (255, 255, 255), 10, lineType)  # 印出文字
                    if left_right == "right":
                        cv2.putText(img, text, (230, 120), fontFace, 5, (255, 255, 255), 10, lineType)  # 印出文字
                    hand_judge = hand_judge + 1
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                # 將節點和骨架繪製到影像中
